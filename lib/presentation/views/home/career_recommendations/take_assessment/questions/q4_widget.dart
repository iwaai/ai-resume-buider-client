        ...
        return Column(
          ...
          children: [
            TakeAssessmentQuestion(
              question: state.careerRecommendationQuestions[3].question,
            ),
            SizedBox(
              height: 8.h,
            ),
            PrimaryDropdown(
              ...
              initialValue:
                  state.answers[state.careerRecommendationQuestions[3].id],
              hintText: 'Select Your Answer',
              maxHeightAfterOpen: 250.h,
              onChanged: (newValue) {
                context.read<TakeAssessmentBloc>().add(
                      SetAnswer(
                          state.careerRecommendationQuestions[3].id, newValue),
                    );
              },
            ),
            const Spacer(),
            PrimaryButton(
                onPressed: () {
                  if (state
                          .answers[state.careerRecommendationQuestions[3].id] !=
                      null) {
